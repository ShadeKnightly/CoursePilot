...
export const getAllCourses = async () => {
    ...
    // Allow courses to be returned even if they are not yet mapped in CourseList/Programs
    const result = await pool.request().query(
        "SELECT c.courseID, c.courseCode, c.CourseName, c.term, c.dateRange, p.title, c.c_Description, p.programID " +
        "FROM Courses c " +
        "LEFT JOIN CourseList cl ON c.courseID = cl.courseID " +
        "LEFT JOIN Programs p ON p.programID = cl.programID;"
    );
